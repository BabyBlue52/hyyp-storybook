<template>
    ...
    <div :class="[openSidebar ? '' : 'closed']" class="hyyp-sidebar">
        <div class="sidebar-title">
            ...
            <div v-if="userReq.status === 'rejected'">
                <p class="rejected">Declined</p>
            </div>
            <div v-else></div>
        </div>
        ...
        <div class="button-row">
            <Button title="Accept" @click="() => nextTick((acceptReq) => forceUpdate)" />
            <InlineButton title="Decline" @click="declineReq" />
        </div>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            userReq: {
                ...
                sent: "3:10:22 EST, January 9th, 2024",
                status: ""
    ...
    methods: {
        ...
        acceptReq() {
            nextTick(() => {
                this.userReq.status = 'accepted';
                window.alert(this.userReq.status)
            })
        },
        declineReq() {
            nextTick(() => {
                this.userReq.status = 'declined'
                window.alert(this.userReq.status)
            })
        }
    },
